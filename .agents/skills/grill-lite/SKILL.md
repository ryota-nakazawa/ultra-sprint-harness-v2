...
ユーザーが「一気通貫で実施」「途中で止めない」「承認待ちにしない」など、チケット化から実装・検証・レビューまでの連続実行を明示した場合は、`Execution Mode: Uninterrupted` と記録する。このモードでは後続スキルの計画・チケット内訳・テスト継ぎ目の提示は**記録と共有のために行うが、承認待ちで停止しない**。

このモードでも、不可逆操作、本番のセキュリティ判断、外部送信、契約・法令・大きなコスト、またはユーザーの選択が必要な仕様上の分岐は自動実行しない。発見したP1相当の不具合は、同じチケット内で修正・再テスト・再レビューを続け、解消不能な場合だけエスカレーションする。
...
- Next flow: 主成果物に対応する後続フロー。`Webアプリ` は `/webapp-design` で `Design Brief` を確認してから既存の `/to-spec` 以降の流れへ渡す
- Source location: `Webアプリ` の場合は `apps/<kebab-case-app-name>/`。それ以外は該当する成果物の配置を記載する
...
`Interactive` または `Uninterrupted`。連続実行の明示指示がない場合は `Interactive`。

## Handoff to Spec

`Harness Improvement` の場合にだけ出力する。
```

`Harness Improvement` の場合は、`/to-spec` に渡す。

主成果物が `Webアプリ` の `Project Delivery` の場合は、上記の `## Handoff to Spec` を出力せず、代わりに次の節を出力する。

```markdown
## Handoff to Webapp Design

`/webapp-design` に渡して `Design Brief` を確認する。`Design Brief` が確定してから `/to-spec` に渡す。
```
...
`Ready for Spec` が `Yes` の場合、`Harness Improvement` は成果物を現在の会話コンテキストとして `/to-spec` に渡すよう案内する。主成果物が `Webアプリ` の `Project Delivery` は `/webapp-design` に渡し、`Design Brief` の確定後に `/to-spec` へ進む。その他の `Project Delivery` は、`Artifact Routing` の `Next flow` を案内する。`to-spec` は追加インタビューをせず、この成果物の確定事項・仮定・未解決事項をもとに仕様化する。
